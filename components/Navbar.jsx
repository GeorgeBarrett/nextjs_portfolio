...
const Navbar = () => {
    ...
    useEffect(() => {
        const sub = addEventListener('scroll', () => setPageScroll(window.scrollY >= 130))
    ...
    return (
        <div className={`w-full h-20 z-10 fixed bg-white text-black duration-300 ease-in ${pageScroll && "bg-[#fefefea7]"}`}>
                ...
                >    
                    ...
                    <div>
                        <div className="flex w-full items-center justify-between">
                            <Link href="/#home">
                                <h2 onClick={() => setNavigation(false)} className="text-3xl font-bold uppercase underline underline-offset-2 tracking-wider cursor-pointer">George</h2>
                            </Link>
                    ...
                    <div className="mt-24 flex flex-col h-fit gap-20">
                        <ul className="w-5 uppercase">
                            ...
                            {links.map(({id, link}) => (
                                <Link key={id} href={`/#${link}`}>
                                    <li onClick={() => setNavigation(false)} className="py-4 text-2xl tracking-wider cursor-pointer">{link}</li>
                                </Link>
                            ))}
